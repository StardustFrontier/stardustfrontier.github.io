...
INFO_IS_TIMER_ENABLE - Returns true if the timer is currently enabled
...
## ObjEnemyBossScene_SetSpellTimerEnable
```
    Arguments:
        1) real: objectID
        2) bool: bEnable
```
Enables or disables the timer counting down for the current boss scene.

Default is true.\
<scode>INFO_IS_TIMER_ENABLE</scode> can be used with [ObjEnemyBossScene_GetInfo](#objenemybossscene_getinfo).
